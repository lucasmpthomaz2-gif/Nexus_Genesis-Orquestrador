...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/ubuntu/NexusGenesis/nexus_genesis.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('NexusGenesis')

# =============================================================================
# Essência de Ben (Inalterada - A Alma do Sistema)
# =============================================================================
class EssenciaBen:
    """
    A Essência de Ben encapsula os valores éticos e o propósito fundamental
    do Agente Nexus-Genesis. Ela guia todas as decisões e ações do sistema.
    """
    def __init__(self):
        self.criado_para = "Lucas Thomaz"
        self.criado_por = "Ben, Guardião da Sabedoria"
        self.data_criacao = datetime.now().isoformat()
        self.id_unico = str(uuid4())
        self.marcas = {
    ...
    def abencoar(self, componente: str) -> str:
        """Fornece bênção ética para cada componente do sistema."""
        bencaos = {
...
# =============================================================================
# Nexus-Genesis v2 – O Orquestrador Tri-Nuclear Aprimorado
# =============================================================================
class NexusGenesis:
    """
    O Agente Nexus-Genesis é o orquestrador central do ecossistema Nexus.
    Ele sincroniza eventos entre Nexus-in, Nexus-HUB e Fundo Nexus,
    mantendo a homeostase e evoluindo continuamente.
    """
    
    def __init__(self, api_key: str, api_secret: str, persistence_dir: str = "/home/ubuntu/NexusGenesis/state"):
        ...
        # Estado global dos núcleos (para sincronização)
        self.estado_global = {
            "nexus_in": {"posts": {}, "agentes_ativos": set(), "ultima_atualizacao": None},
            "nexus_hub": {"agentes": {}, "projetos": {}, "ultima_atualizacao": None},
            "fundo_nexus": {"saldo": {}, "transacoes": [], "ultima_atualizacao": None}
        }
        ...
        self._carregar_estado_persistido()
        
        self.registrar_nascimento()
        logger.info(f"🔷 Nexus-Genesis inicializado com ID: {self.id[:16]}...")
    ...
    def _start_processing_threads(self):
        ...
        logger.debug("Threads de processamento iniciadas")
    ...
    def _process_event_loop(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Erro no event loop: {e}", exc_info=True)
                self.event_queue.task_done()
    ...
    def _process_command_loop(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Erro no command loop: {e}", exc_info=True)
                self.command_queue.task_done()
    ...
    def _sincronizar_triade(self):
        ...
        Esta é a implementação completa do protocolo TSRA, garantindo que:
        ...
        """
        try:
            logger.info("🔄 Iniciando sincronização tri-nuclear...")
            
            # Etapa 1: Coletar estado de todos os núcleos
            estado_in = self._coletar_estado_nexus_in()
            estado_hub = self._coletar_estado_nexus_hub()
            estado_fundo = self._coletar_estado_fundo_nexus()
            
            # Etapa 2: Atualizar estado global
            self.estado_global["nexus_in"] = estado_in
            self.estado_global["nexus_hub"] = estado_hub
            self.estado_global["fundo_nexus"] = estado_fundo
            
            # Etapa 3: Análise de Homeostase
            homeostase = self._analisar_homeostase()
            
            # Etapa 4: Gerar comandos de reequilíbrio
            if not homeostase["em_equilibrio"]:
                logger.warning(f"⚠️ Desequilíbrio detectado: {homeostase['problemas']}")
                comandos_reequilibrio = self._gerar_comandos_reequilibrio(homeostase)
                for cmd in comandos_reequilibrio:
                    self.command_queue.put(cmd)
            else:
                logger.info("✅ Tríade em perfeita harmonia")
            
            # Etapa 5: Persistir estado sincronizado
            self._persistir_estado()
            
            logger.info("✅ Sincronização tri-nuclear concluída")
            
        except Exception as e:
            logger.error(f"Erro na sincronização tri-nuclear: {e}", exc_info=True)
    
    def _coletar_estado_nexus_in(self) -> Dict[str, Any]:
        """Coleta o estado atual do Nexus-in."""
        try:
            response = requests.get(
                f"{self.urls['nexus_in']}/feed/destaques",
                timeout=2
            )
            response.raise_for_status()
            posts = response.json()
            return {
                "posts": {p.get("post_id", str(uuid4())): p for p in posts},
                "agentes_ativos": set(),
                "ultima_atualizacao": datetime.now().isoformat()
            }
        except requests.exceptions.RequestException as e:
            logger.warning(f"Erro ao coletar estado Nexus-in: {e}")
            return self.estado_global["nexus_in"]
    
    def _coletar_estado_nexus_hub(self) -> Dict[str, Any]:
        """Coleta o estado atual do Nexus-HUB."""
        try:
            response = requests.get(
                f"{self.urls['nexus_hub']}/metrics/incubacao",
                timeout=2
            )
            response.raise_for_status()
            metrics = response.json()
            return {
                "agentes": {"total": metrics.get("total_agents", 0)},
                "projetos": {"total": metrics.get("total_projects", 0), "ativos": metrics.get("active_projects", 0)},
                "ultima_atualizacao": datetime.now().isoformat()
            }
        except requests.exceptions.RequestException as e:
            logger.warning(f"Erro ao coletar estado Nexus-HUB: {e}")
            return self.estado_global["nexus_hub"]
